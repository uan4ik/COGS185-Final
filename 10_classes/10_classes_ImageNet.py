...
def load_dataset():
    return pickle.load(open("Mini_10_classes_ImageNet.p", "rb"))
...
def build_mlp(input_var=None):
    ...
    l_out = lasagne.layers.DenseLayer(
            l_hid2_drop, num_units=num_labels,
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.InputLayer(shape=(None, 3, 500, 300),
                                        input_var=input_var)
    ...
    network = lasagne.layers.Conv2DLayer(
            network, num_filters=32, filter_size=(25, 25),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
